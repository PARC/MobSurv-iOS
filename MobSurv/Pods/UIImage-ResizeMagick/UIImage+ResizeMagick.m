...
- (UIImage *)resizedImageByMagick:(NSString *)spec {
	if ([spec hasSuffix:@"!"]) {
		...
		NSUInteger width = (unsigned)[[widthAndHeight objectAtIndex:0] longLongValue];
		NSUInteger height = (unsigned)[[widthAndHeight objectAtIndex:1] longLongValue];
	...
	if ([widthAndHeight count] == 1) {
		return [self resizedImageByWidth:(unsigned)[spec longLongValue]];
	}
	if ([[widthAndHeight objectAtIndex:0] isEqualToString:@""]) {
		return [self resizedImageByHeight:(unsigned)[[widthAndHeight objectAtIndex:1] longLongValue]];
	}
